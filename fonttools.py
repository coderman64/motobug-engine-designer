...
def quickRenderTextCenter(rend,font,text,x,y):
    # convert text to a bytestring
    text = bytes(str(text),"ASCII")
    ...
    textw, texth = ctypes.c_int(),ctypes.c_int()
    TTF_SizeText(font,text,textw,texth)
    rect.x, rect.y, rect.w, rect.h = round(x-textw.value/2),round(y-texth.value/2),textw.value, texth.value
...
def preRenderTextCenter(rend,font,text,x,y):
    # convert text to a bytestring
    text = bytes(str(text),"ASCII")
    ...
    # render the text and convert it to a texture
    textSurface = TTF_RenderText_Blended(font,text,fontColor)
    textTex = SDL_CreateTextureFromSurface(rend, textSurface)
    SDL_FreeSurface(textSurface)
...
def renderTextCenter(rend,font,text,x,y,textTex):
    # convert text to a bytestring
    text = bytes(str(text),"ASCII")
